...
def categorical_parse_params_column(params, min_prob=0):
    ...
    distr = Categorical(probs=params, validate_args = False)
    return distr
    ...
    def forward(self, groundtruth, distr_params, mask):
        ...
        for i, size in enumerate(self.one_hot_max_sizes):
            ...
            if size <= 1:
                ...
                mask_col = mask_col * (~ nan_mask).float() # [1 - torch.isnan(v)] change to [~ torch.isnan(v)]
    ...
    def forward(self, input):
        ...
        for i, size in enumerate(self.one_hot_max_sizes):
            if size <= 1:
                ...
                # print(out_col.shape)
                # print(out_col)
                # print(torch.arange(n).long(), cat_idx.long())
